...
Одновременно навык проверяет, есть ли на [opensubtitles.org](https://www.opensubtitles.org/) готовые русские субтитры под тот же релиз. Если они найдены, перед переводом он предлагает каждый вариант, показывает ссылку и значение поля `translator`. Если поле пустое, указывается `translator не указан`. Готовые русские субтитры не используются автоматически.
