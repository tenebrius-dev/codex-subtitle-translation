# Codex Subtitle Translation

[Русский](README.md) | [English](README.en.md)

Навык Codex для контекстного перевода субтитров на русский язык.

## Возможности

Навык работает с одним видеофайлом или папкой с фильмом/сериалом и:

1. находит видеофайл или список серий;
2. ищет на [opensubtitles.org](https://www.opensubtitles.org/) английские субтитры под точный релиз;
3. проверяет название, сезон, серию, разрешение, источник, кодек, release group и тайминги;
4. изучает контекст фильма или сериала, персонажей и терминологию;
5. переводит субтитры на русский язык;
6. сохраняет номера реплик, таймкоды, количество строк и оформление SRT;
7. проверяет готовый файл;
8. сохраняет результат рядом с видео с тем же именем.

Если в указанной папке нет видеофайлов в корне, навык автоматически проверяет все вложенные папки и обрабатывает найденные там видеофайлы. Перечислять подпапки вручную не нужно.

Проверка выполняется на живой странице [opensubtitles.org](https://www.opensubtitles.org/) в браузере. Результаты поисковиков, кэшированные страницы и ошибки загрузки не считаются доказательством наличия или отсутствия субтитров.

Одновременно навык проверяет, есть ли готовые русские субтитры под тот же релиз. Если они найдены, перед переводом он показывает каждый вариант отдельно: ссылку на страницу субтитров, точное имя файла, язык, тип, релиз, тайминг, uploader и значение поля `translator`. Если поле пустое, указывается `translator не указан`. После этого навык ждёт явное решение `продолжить` или `остановиться`; готовые русские субтитры не выбираются автоматически.

## Быстрый запуск

Достаточно написать в Codex:

```text
Сделай русские субтитры для серии /path/to/series-folder
```

Или для одного файла:

```text
Сделай русские субтитры для /path/to/movie.mkv
```

Повторять инструкцию и явно указывать `$codex-subtitle-translation` не обязательно: навык настроен на автоматический запуск по такому запросу.

Для принудительного запуска:

```text
Используй $codex-subtitle-translation.
Сделай русские субтитры для /path/to/series-folder
```

## Удаление звуков и SDH

Для очистки готовых субтитров напишите:

```text
Убрать звуки
```

или:

```text
Убрать SDH
```

Навык сначала сохраняет исходный файл как `имя.SDH.srt`, затем очищает исходный SRT: удаляет cue только со звуковыми описаниями в `[...]`, cue с единственным знаком `♪`, убирает блоки `[...]` из оставшихся реплик и перенумеровывает cue.

## Извлечение субтитров из видео

Если нужно использовать встроенную английскую дорожку из самого MKV, напишите:

```text
Используй субтитры из файла видео
```

В этом режиме OpenSubtitles не используется. Навык проверяет встроенные дорожки, выбирает английскую текстовую дорожку и сохраняет её как:

```text
/path/Example.Release.mkv
/path/Example.Release.eng.srt
```

Добавляется только `.eng` перед `.srt`; полное имя релиза сохраняется. Если английских дорожек несколько, навык попросит выбрать нужную. PGS и VobSub являются графическими форматами и требуют OCR для получения текстового SRT.

## Имена файлов

Для видео:

```text
/path/Example.Movie.2025.1080p.WEB-DL.mkv
```

результат сохраняется как:

```text
/path/Example.Movie.2025.1080p.WEB-DL.srt
```

Имя релиза сохраняется полностью. Суффиксы `.ru`, `.rus`, `.translated` и подобные не добавляются.

## Правила

- Если найдено несколько разных релизов, навык запрашивает уточнение.
- Субтитры от другого релиза не подставляются молча.
- Найденные русские субтитры предлагаются с URL и значением поля `translator`.
- Если в корне указанной папки нет видеофайлов, проверяется всё дерево вложенных папок.
- При обработке папки каждая серия сохраняется сразу после перевода и проверки.
- Видео не переименовывается, не перемещается и не перекодируется.
- Если пользователь передаёт готовый SRT или ссылку, они используются как исходник после проверки соответствия видео.
- Для извлечения встроенной дорожки используется `mkvextract` из MKVToolNix.
- Для очистки SDH используется `scripts/remove_sdh.py`.
- Для проверки совместимости английского и русского кандидатов используется `scripts/compare_srt_compatibility.py`.

## Ручная проверка SRT

Встроенный скрипт проверяет количество реплик, номера, таймкоды, количество строк и теги оформления:

```bash
python3 ~/.codex/skills/codex-subtitle-translation/scripts/validate_srt.py \
  "/path/source.en.srt" \
  "/path/translated.srt"
```

Скрипт не требует сторонних Python-библиотек.

## Установка из GitHub

```bash
git clone https://github.com/tenebrius-dev/codex-subtitle-translation.git \
  ~/.codex/skills/codex-subtitle-translation
```

После установки перезапустите Codex или начните новый запрос, чтобы навык появился в списке доступных навыков.
